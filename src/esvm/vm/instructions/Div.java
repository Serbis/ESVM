...
public class Div extends Instruction{
    ...
    public void exec() throws IncompatibleTypesException, StackOverflowException {
        try {
            StackObject a = Global.getInstance().memoryManager.pop();
            StackObject b = Global.getInstance().memoryManager.pop();
            boolean fl = false; //Флаг операции с плавающей точкой
            //Если хоть один операнд флоат, то результат всегда будет так же флоат
            if (a.stackDataType.equals(StackObject.StackDataType.FLOAT) || b.stackDataType.equals(StackObject.StackDataType.FLOAT))
                fl = true;
